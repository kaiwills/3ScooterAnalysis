...
def joint_positions_calculator(foot_length, lower_leg_length, thigh_length, torso_height, upper_arm_length, forearm_length, angles_in):
    
    foot_length = foot_length 
    lower_leg = lower_leg_length
...
def calc_joint_positions(data, gender, percentile, angles_in):
    
    foot_length = data['FootLength'][f'{gender}{percentile}']  
    lower_leg = data['LowerLegLength'][f'{gender}{percentile}']
    thigh_length = data['ThighLength'][f'{gender}{percentile}']
    torso_height = data['TorsoHeight'][f'{gender}{percentile}'] 
    upper_arm = data['UpperArmLength'][f'{gender}{percentile}'] 
    forearm = data['ForearmLength'][f'{gender}{percentile}']
...
def calc_constrained_angles(ankle_range, knee_range, shoulder_range, hip_range, elbow_range, back_inclination):
    """
    ...
    """
    foot_angle = 180
    lowerleg_angle = ankle_range 
    thigh_angle = 180 - (knee_range - lowerleg_angle)
    torso_angle = back_inclination
    hip = torso_angle + (knee_range - ankle_range)
    upperarm_angle = 190 + torso_angle + shoulder_range
    elbow = torso_angle + shoulder_range
    forearm_angle = 0
...
def calc_joint_positions_editted(data, gender, percentile, angles):    
    ...
    torso_height = data['TorsoHeight'][f'{gender}{percentile}'] 
    upper_arm = data['UpperArmLength'][f'{gender}{percentile}'] 
    forearm_length = data['ForearmLength'][f'{gender}{percentile}']

    dimensions = [foot_length, lower_leg, thigh_length, torso_height, upper_arm, forearm_length]
...
def plot_skeleton(joints, dists = True, gender = 'Man', percentile = '50', save=False, overlay=False):
    ...
    if dists:
        ...
        if overlay:
            # Horizontal A
            ax.annotate('', 
                xy=(joint_x[0], joint_y[4] + 500),  # Start of the line (xy)
            ...
            ax.vlines(joint_x[0], joint_y[0], joint_y[4]+500 , linestyle='--', colors='black', linewidth=0.5)
            ax.vlines(joint_x[6], joint_y[6], joint_y[4]+500 , linestyle='--', colors='black', linewidth=0.5)
            ax.text((joint_x[0] + joint_x[6]) / 2, joint_y[4] + 530, "Distance A", ha='center', va='bottom', fontsize=10)
            ...
            # Horizontal A
            ax.annotate('', 
                xy=(joint_x[0], joint_y[4] + 200),  # Start of the line (xy)
            ...
            ax.annotate('', 
                ...
                xytext=(joint_x[6], joint_y[4] + 200),  # End of the line (xytext)
                arrowprops=dict(facecolor='black', arrowstyle='->'))  # Arrow at the end
            ax.vlines(joint_x[0], joint_y[0], joint_y[4]+200 , linestyle='--', colors='black', linewidth=0.5)
            ax.vlines(joint_x[6], joint_y[6], joint_y[4]+200 , linestyle='--', colors='black', linewidth=0.5)
            ax.text((joint_x[0] + joint_x[6]) / 2, joint_y[4] + 230, "Distance A", ha='center', va='bottom', fontsize=10)
    ...
    image = Image.open(r'assets/Man_Seated.jpeg').convert('RGBA')
...
def calculator_plot(joints, dists = True, save=False):
    ...
    plt.title(f"2D Human Skeleton")
    
    if save:
        plt.savefig(f'Caclulated_Dimensions_2D.png')
